...
(define (eval exp env) 
 (cond  ((self-value? exp)   exp)
		...
		;; install other specail forms here
		;;((and-stmt? exp)  (eval-and-stmt exp env))
		;;((or-stmt? exp)  (eval-or-stmt exp env))
		;;
		
		;; procedure can be in arguments, operator, return simplify the eval here.
		((lambda? exp)  (make-procedure  (lambda-para exp)
														   (lambda-body exp)
														   env))
		((begin-stmt? exp)   (eval-sequence (begin-actions exp) env))
		((cond? exp)  (eval (cond->if exp) env))
		((procedure? exp) (applyx (eval (operator exp) env)  
