;; my simple draft of lazy evaluator
...
(define (eval exp env) 
 (cond  ((self-value? exp)   exp)
		...
		((pair? exp)   (applyx (actual-value (operator exp) env)  
		                                     (operand exp)
											  env))
...
(define (applyx procedure arguments env)
 (cond ((primitive-procedure? procedure)  (apply-primitive-procedure procedure (list-of-actual-values arguments env)))
       ((compound-procedure? procedure)  (eval-sequence (procedure-body procedure)
	                                                    (extend-environment (procedure-parameters procedure)
														                    (list-of-delay-values arguments env)
...
(define (list-of-actual-values exps env)
   (if (no-operands? exps)
       '()
	   (cons (actual-value (first-operand exps) env)
	         (list-of-actual-values (rest-operand exps) env))))
 
(define (list-of-delay-values exps env)
  (if (no-operands? exps)
	  '()
	  (cons (delay-it (first-operand exps) env)
       	    (list-of-delay-values (rest-operand exps) env))))
...
; (define (force-it object)               ;; this need to force to bottom, not only one step
  ; (if (thunk? object)
      ; (actual-value (thunk-exp object) (thunk-env object))
	   ; object))
...
;;memorized version of force-it

(define (memorized? object)
  (tagged-list? object 'memorized))
  
(define (value-of-memorized object)
 (cadr object))  
  
 (define (force-it object)
   (cond ((memorized? object) (value-of-memorized object))
         ((thunk? object) 
		  (let ((result (actual-value (thunk-exp object) (thunk-env object))))
             (set-car! object 'memorized)
             (set-car! (cdr object) result)
             (set-car! (cddr object) '())
			 result))          
         (else object)))		  
...
(define (eval-if exp env)
 (if (true? (actual-value (if-pred exp) env))
...
(define (driver)
  ...
  (let ((input (read)))
	...
	(display input)
	(newline)
    (let ((result (actual-value input env0)))	
     (newline)	 
